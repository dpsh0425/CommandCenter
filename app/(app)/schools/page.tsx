  ...
  return (
    <main className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex gap-2 mb-4">
        <a href="/schools" className={`px-3 py-1 rounded-full text-sm border ${!country ? "bg-brass text-ink font-medium" : ""}`}>All</a>
        {countries.map((c) => (
          <a key={c} href={`/schools?country=${c}`} className={`px-3 py-1 rounded-full text-sm border ${country === c ? "bg-brass text-ink font-medium" : ""}`}>{c}</a>
        ))}
      </div>
