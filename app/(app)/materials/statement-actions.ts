...
async function owner() {
  ...
  if (!user || user.id !== OWNER_USER_ID) throw new UserError("Only the workspace owner can change statements.");
...
export async function createStatement(input: { kind: string; schoolId?: string | null; fromId?: string | null; title?: string }): Promise<ActionResult<string>> {
  return toResult(async () => {
    ...
    if (input.fromId) {
      ...
      if (!src) throw new UserError("The draft to copy from was not found.");
      body = src.body;
      prompt = src.prompt;
...
export async function updateStatementMeta(id: string, f: { title?: string; kind?: string; prompt?: string | null; wordLimit?: number | null; charLimit?: number | null }): Promise<ActionResult> {
  return toResult(async () => {
    const { supabase } = await owner();
    const row: Record<string, unknown> = {};
    if (f.title !== undefined) {
      if (!f.title.trim()) throw new UserError("The title can't be empty.");
      row.title = f.title.trim();
    }
    if (f.kind !== undefined) {
      if (!isStatementKind(f.kind)) throw new UserError("Unknown statement type.");
      row.kind = f.kind;
    }
...
export async function saveStatementBody(id: string, body: string, baseVersion: number): Promise<SaveResult> {
  ...
  try { ({ supabase } = await owner()); } catch (e) {
    if (e instanceof UserError) return { ok: false, reason: "error", message: e.message };
    throw e;
  }
...
export async function setStatementStatus(id: string, next: string): Promise<ActionResult> {
  return toResult(async () => {
    const { supabase } = await owner();
    if (!isStatementStatus(next)) throw new UserError("Unknown status.");
    const { data: cur } = await supabase.from("statements").select("sent_on, school_id").eq("id", id).single();
    if (!cur) throw new UserError("Statement not found.");
...
export async function saveSnapshot(id: string, note?: string): Promise<ActionResult> {
  return toResult(async () => {
    const { supabase, userId } = await owner();
...
export async function restoreSnapshot(snapshotId: string): Promise<ActionResult<{ body: string; words: number; version: number }>> {
  return toResult(async () => {
    ...
    if (!snap) throw new UserError("Version not found.");
    ...
    const { data: updated, error } = await supabase
      ...
      .select("body_version");
    if (error) throw new Error(error.message);
    if (!updated || updated.length === 0) throw new UserError("The statement changed while restoring. Try again.");
    refresh(snap.statement_id, cur.school_id);
    return { body: clean, words, version: updated[0].body_version as number };
  });
}

export async function deleteSnapshot(snapshotId: string, statementId: string): Promise<ActionResult> {
  return toResult(async () => {
    const { supabase } = await owner();
